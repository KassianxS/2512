...
Сначала система проверяет, было ли передано более трех аргументов. Если да, то второй и третий аргументы извлекаются и сохраняются в переменные с именами idx и v4.
Затем проводится валидация: каждый символ входных данных должен принадлежать к определенному алфавиту.
Ключевой этап — это функция decode. Она последовательно обрабатывает каждый символ строки, применяя к нему преобразование, выполняемое функцией transform().
Сама функция transform() осуществляет аффинное преобразование. Для каждого символа используется следующая формула для получения нового символа: new_char = alph[(a3 + a2 * old_idx) % 29]


также написал код который позволяет получить правильные значения a2 a3, перебирая все варианты
...
ciphertext = "dix_gyhiiz}xdduah}puvyhn}u}pxa}tnbfh}ozbc"
...
def decode(s, a2, a3):
    ...
    for c in s:
        if c not in idx:
            return None
        i = idx[c]
        res += alph[(a3 + a2 * i) % MOD]
    return res

for a2 in range(MOD):
    for a3 in range(MOD):
        out = decode(ciphertext, a2, a3)
        if out and out.startswith("flag{"):
            print(f"[+] a2={a2}, a3={a3}")
            print(out)
```
...
<img width="1354" height="245" alt="Снимок экрана 2025-12-21 130334" src="https://github.com/user-attachments/assets/aacad486-a405-44b5-bfdf-d31c5925ca20" />
...
Смотрим, что делает программа: она берет то, что мы ей дали (аргумент), и кладет в v5. Еще есть зашифрованная строка Str (cPK}[aYr^@ZZR`C]TBP_\\Y_U).

Потом идет проверка.

Дальше запускается функция, у которой название говорит само за себя. Если посмотреть на ее упрощенный код, то видно, что она просто меняет местами соседние цифры ввода: i-ю с (i+1)-й.

После этого программа проверяет, стал ли наш массив v8 отсортированным (то есть [1, 2, ..., 10]).

Если v4 равен 1 (что значит, что массив теперь в правильном порядке), то происходит вот что:

Каждый байт флага "перемешивается" с соответствующей цифрой из нашего ввода с помощью XOR.

Чтобы получить флаг, нам нужно найти такую последовательность цифр, которая превратит наш v8 в отсортированный массив.

код:
```c
def decrypt_flag():
    # Начальный массив и его копия
    original = [8, 7, 5, 4, 1, 3, 2, 6, 9, 10]
    work = original.copy()
    swap_sequence = []

    # Генерируем последовательность индексов свапов, чтобы отсортировать массив
    for _ in range(len(work)):
        for idx in range(len(work)-1):
            if work[idx] > work[idx+1]:
                work[idx], work[idx+1] = work[idx+1], work[idx]
                swap_sequence.append(idx)

    # Превращаем индексы свапов в строку цифр
    input_code = ''.join(map(str, swap_sequence))

    # Проверяем, что последовательность корректно сортирует массив
    test_array = original.copy()
    for idx in swap_sequence:
        test_array[idx], test_array[idx+1] = test_array[idx+1], test_array[idx]
    assert test_array == list(range(1, 11)), "Sorting failed!"

    # Зашифрованная строка из бинарника
    cipher = b"cPK}[aYr^@ZZR`C]TBP_\\Y_U"

    # Расшифровка флага через XOR с цифрами ввода
    flag_bytes = bytearray()
    for i, byte in enumerate(cipher):
        flag_bytes.append(byte ^ ord(input_code[i % len(input_code)]))

    print("Input digits:", input_code)
    print("FLAG:", flag_bytes.decode())

decrypt_flag()
```
вывод
<img width="1264" height="172" alt="Снимок экрана 2025-12-21 140715" src="https://github.com/user-attachments/assets/c6db3664-40cb-475f-a826-e11ace896487" />
